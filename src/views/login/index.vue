...
import {checkoutMoblie, userRegister, login, sendPhoneCode, changePassWord, loginByOpenId} from 'api/login'
...
export default {
  ...
  methods: {
    ...
    register () {
      if (this.choice) {
        ...
        if (this.phoneNum && this.messages && this.setPassword) {
          userRegister({mobile: this.phoneNum, smsToken: this.messages, inviteFrom: this.invitCode, password: this.setPassword, openid: this.$route.query.openid}).then(response => {
            var data = response.data
            var code = String(data.code)
    ...
    userlogin () {
      if (this.password) {
        login({mobile: this.phoneNum, password: this.password, openid: this.$route.query.openid}).then(response => {
          var data = response.data
          var code = String(data.code)
          if (code === '200') {
